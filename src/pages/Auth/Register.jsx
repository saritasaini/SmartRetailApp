...
import { Mail, Lock, Store, User, Phone, Loader2, AlertCircle, MapPin } from 'lucide-react';
...
export default function Register() {
  ...
  if (success) {
    return (
      ...
      >
        ...
        <p className="text-text-secondary mb-6">Your retailer account has been created successfully. Please wait for company approval before logging in.</p>
  ...
  return (
    ...
    >
      <h2 className="text-2xl font-bold text-text-primary mb-6 text-center">Register Shop</h2>
      ...
      {!companyId && !success && (
        <div className="mb-4 p-3 rounded-lg bg-brand-honey/10 border border-yellow-500/50 flex items-center gap-2 text-brand-honey text-sm">
          <AlertCircle size={16} />
          <span>Warning: You are registering without a valid company link. Your account will not be linked.</span>
        </div>
      )}

      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-[#6B4226] mb-1">Shop Name</label>
